...
def load_db():
    ...
    Returns:
        (conn, q) where:
          - conn is a live SQLite connection
          - q(sql, params=None) is a shorthand query function

    Example:
        conn, q = load_db()
        matches = q("SELECT * FROM Match LIMIT 5")
    """
    # 1) Get DB path from Kaggle
    db_path = get_kaggle_db_path()

    # 2) Open connection
    conn = connect_to_db(db_path)

    # 3) Local helper that uses the shared connection
